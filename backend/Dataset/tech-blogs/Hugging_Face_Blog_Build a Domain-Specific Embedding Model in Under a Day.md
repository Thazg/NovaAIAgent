...
URL Source: https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune

Published Time: 2026-03-20T19:38:16.078Z

Markdown Content:
[Back to Articles](https://huggingface.co/blog)

[![Image 1: Steve Han's avatar](https://huggingface.co/avatars/9e3e29394381d2ce973910e3cba1fe44.svg)](https://huggingface.co/steve-nvidia)

[![Image 2: Rucha Apte's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/67329fe54c2f18a60e5c7705/hhaORAzSD3WMCmOKVDokx.jpeg)](https://huggingface.co/ruchaa01)

[![Image 3: Sean Sodha's avatar](https://huggingface.co/avatars/cdd9a4229264f355625df345786b2a08.svg)](https://huggingface.co/ssodha-nv)

[![Image 4: Oliver Holworthy's avatar](https://huggingface.co/avatars/ae8826dc73489e90069249aac3f6216b.svg)](https://huggingface.co/nvidia-oliver-holworthy)
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%94%97quick-links-to-dataset-and-code) 🔗Quick Links to Dataset and Code:

1.   [Embedding Model](https://huggingface.co/nvidia/llama-nemotron-embed-1b-v2)
2.   [GitHub](https://github.com/NVIDIA-NeMo/Nemotron/tree/main/src/nemotron/recipes/embed)
3.   [Synthetic dataset on NVIDIA’s public documents](https://huggingface.co/datasets/nvidia/Retrieval-Synthetic-NVDocs-v1)

### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%A7%91%F0%9F%92%BBopen-source-projects-recipe-integrates) 🧑‍💻Open Source Projects Recipe Integrates:

1.   [NeMo Data Designer](https://github.com/NVIDIA-NeMo/DataDesigner) for synthetic data generation 
2.   [NeMo Automodel](https://github.com/NVIDIA/NeMo-Automodel) for embedding model training 
3.   [BEIR](https://github.com/beir-cellar/beir) for Information retrieval evaluation 
4.   [NeMo Export-Deploy](https://github.com/NVIDIA/NeMo-Export-Deploy) for ONNX/TensorRT conversion 
5.   [NVIDIA NIM](https://developer.nvidia.com/nim) for production inference serving

### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%93%8Bprerequisites) 📋Prerequisites:

*   A directory of domain documents (text files - .txt, .md, or similar) 
*   A valid NVIDIA API key (free at [build.nvidia.com](https://build.nvidia.com/)) 
*   NVIDIA Ampere GPU or newer with at least 80GB memory (with [Compute Capability](https://developer.nvidia.com/cuda-gpus)>= 8.0) 
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%E2%9A%99%EF%B8%8Fsetup) ⚙️Setup
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%93%9A-step-1-generate-training-data-from-documents) 📚 Step 1: Generate Training Data from Documents
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#how-does-it-work) How does it work?
...
[![Image 5: sdg_pipeline](https://cdn-uploads.huggingface.co/production/uploads/662a9c2154a9f1037104a075/84IMYChKX4twWnC4U6iCQ.png)](https://cdn-uploads.huggingface.co/production/uploads/662a9c2154a9f1037104a075/84IMYChKX4twWnC4U6iCQ.png)

### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#what-does-the-output-look-like) What does the output look like?
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%E2%9B%8F%EF%B8%8F-step-2-mine-hard-negatives-and-why-they-matter) ⛏️ Step 2: Mine Hard Negatives (and Why They Matter)

If you train an embedding model with only positive pairs (query + correct document), it learns to distinguish obviously different documents but fails on the hard cases — passages that _look_ relevant but are not the right answer. In a real retrieval system, these near-misses are exactly the documents that cause bad answers. Hard negative mining finds these confusing passages so the model can learn to tell them apart.
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#2a-train--validation--test-split) 2a. Train / Validation / Test Split
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#2b-hard-negative-mining) 2b. Hard Negative Mining
...
4.   Applies a margin filter: any non-positive document scoring _above_ 95% of the minimum positive score is eliminated. This exclusion zone guards against false negatives — unlabeled passages that are so close to the positive they may actually be relevant. 
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#2c-multi-hop-unrolling) 2c. Multi-Hop Unrolling

Multi-hop questions reference multiple positive documents. For example, a question like _"How does the thermal management system in Section 3.2 relate to the power constraints described in Section 5.1?"_ has two positive passages.
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%94%8D-step-3-understand-multi-hop-questions-and-why-they-improve-retrieval) 🔍 Step 3: Understand Multi-Hop Questions and Why They Improve Retrieval

Standard embedding fine-tuning generates one question per passage and trains the model to match them. This works for simple factual lookups, but real users ask complex questions that span multiple documents or sections. If the model has only seen single-hop training data, it will struggle to retrieve _all_ the relevant passages for these complex queries.
...
Each hop is tracked with its own context summary and segment IDs, so the training data preserves the full reasoning chain. After unrolling (Step 2c), each (question, relevant passage) pair becomes an independent training signal, teaching the model that _all_ of these passages are relevant to the multi-hop query.

The fine-tuned model learns to retrieve contextually related documents, not just lexically similar ones.

## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%A7%A0-step-4-fine-tune-the-embedding-model) 🧠 Step 4: Fine-Tune the Embedding Model

```
nemotron embed finetune -c default
```

### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#how-contrastive-learning-works) How contrastive learning works

The training uses a **biencoder architecture** with **contrastive loss**.

[![Image 6: contrastive_learning](https://cdn-uploads.huggingface.co/production/uploads/662a9c2154a9f1037104a075/ntYmgzatUK_Sfn35GCdve.png)](https://cdn-uploads.huggingface.co/production/uploads/662a9c2154a9f1037104a075/ntYmgzatUK_Sfn35GCdve.png)
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#key-hyperparameters) Key hyperparameters
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#checkpoint-frequency) Checkpoint frequency

If `ckpt_every_steps` is omitted from the config, checkpoint frequency is set automatically:
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%93%88-step-5-measure-the-improvement) 📈 Step 5: Measure the Improvement
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#what-if-the-numbers-dont-improve) What if the numbers don't improve?
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%8F%86-real-world-results-atlassian) 🏆 Real-World Results: Atlassian
...
[![Image 7: ATL result](https://cdn-uploads.huggingface.co/production/uploads/662a9c2154a9f1037104a075/hsFtTLz1WfSgaWBdHof82.png)](https://cdn-uploads.huggingface.co/production/uploads/662a9c2154a9f1037104a075/hsFtTLz1WfSgaWBdHof82.png)

## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#%F0%9F%9A%80-step-6-export-and-deploy) 🚀 Step 6: Export and Deploy
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#export-to-onnx--tensorrt) Export to ONNX / TensorRT
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#deploy-with-nvidia-nim) Deploy with NVIDIA NIM
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#verify-deployment-accuracy) Verify deployment accuracy
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#putting-it-all-together) Putting It All Together
...
### [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#expected-time-and-resources) Expected time and resources
...
## [](https://huggingface.co/blog/nvidia/domain-specific-embedding-finetune#try-it-yourself) Try It Yourself
...
**Star the repos** for [Nemotron](https://github.com/NVIDIA/Nemotron), [NeMo Data Designer](https://github.com/NVIDIA/NeMo-Data-Designer) and [NeMo Automodel](https://github.com/NVIDIA/NeMo-Automodel) if you find them useful.
