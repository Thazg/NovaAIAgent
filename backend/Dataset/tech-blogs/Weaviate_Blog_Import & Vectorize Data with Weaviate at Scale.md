...
URL Source: https://weaviate.io/blog/data-import-best-practices

Published Time: 2026-06-18T00:00:00.000Z

Markdown Content:
![Image 1: Import and vectorize data with Weaviate at scale: server-side batching, retries, the blobHash data type, and multimodal ingestion](https://weaviate.io/assets/images/hero-3d3404ba029aa5e8104892b0590e738c.png)
...
## The ingest problem nobody warns you about[​](https://weaviate.io/blog/data-import-best-practices#the-ingest-problem-nobody-warns-you-about "Direct link to The ingest problem nobody warns you about")
...
## Server-side batching[​](https://weaviate.io/blog/data-import-best-practices#server-side-batching "Direct link to Server-side batching")
...
`import weaviatefrom weaviate.classes.init import Authclient = weaviate.connect_to_weaviate_cloud(    cluster_url=WCD_URL,    auth_credentials=Auth.api_key(WCD_API_KEY),)collection = client.collections.get("Products")with collection.batch.stream() as batch:    for row in iter_rows():        batch.add_object(properties=row)        if batch.number_errors > 10:            print("Too many errors, stopping.")            breakif collection.batch.failed_objects:    print(f"{len(collection.batch.failed_objects)} objects failed.")`
...
## Error handling and retries[​](https://weaviate.io/blog/data-import-best-practices#error-handling-and-retries "Direct link to Error handling and retries")
...
`import jsonfrom weaviate.util import generate_uuid5with collection.batch.stream() as batch:    for row in iter_rows():        batch.add_object(            properties=row,            uuid=generate_uuid5(row["source_id"]),        )with open("failed.jsonl", "a") as f:    for obj in collection.batch.failed_objects:        f.write(json.dumps({            "properties": obj.object_.properties,            "error": obj.message,        }) + "\n")`
...
![Image 2: Retry-with-checkpoint flow showing successful writes to Weaviate alongside a failure branch that captures failed_objects to a JSONL file, retries with deterministic UUIDs, and dead-letters repeat failures.](https://weaviate.io/assets/images/retry-checkpoint-flow-72dec1000e327ed02ae6119f33354733.png)
...
## Ingesting through the MCP server[​](https://weaviate.io/blog/data-import-best-practices#ingesting-through-the-mcp-server "Direct link to Ingesting through the MCP server")
...
## Choosing data types before you import[​](https://weaviate.io/blog/data-import-best-practices#choosing-data-types-before-you-import "Direct link to Choosing data types before you import")
...
### `blobHash`: store the embedding, skip the bytes[​](https://weaviate.io/blog/data-import-best-practices#blobhash-store-the-embedding-skip-the-bytes "Direct link to blobhash-store-the-embedding-skip-the-bytes")
...
`{  "properties": [    {      "name": "product_image",      "dataType": ["blobHash"]    }  ]}`
...
![Image 3: Side-by-side diagram comparing blob and blobHash. Both data types send raw bytes through the same vectorizer, but blob persists the full base64 payload while blobHash retains only a 32-byte SHA-256 hash — shrinking a 10 TB image corpus to a few gigabytes stored inside Weaviate.](https://weaviate.io/assets/images/blobhash-vs-blob-1e8b10aada0f234bc156ac74b623e2fc.png)
...
## PDF vectorization without the OCR pipeline[​](https://weaviate.io/blog/data-import-best-practices#pdf-vectorization-without-the-ocr-pipeline "Direct link to PDF vectorization without the OCR pipeline")
...
## Multimodal ingestion: text, image, audio, video[​](https://weaviate.io/blog/data-import-best-practices#multimodal-ingestion-text-image-audio-video "Direct link to Multimodal ingestion: text, image, audio, video")
...
## A checklist before you press go[​](https://weaviate.io/blog/data-import-best-practices#a-checklist-before-you-press-go "Direct link to A checklist before you press go")
...
## Ready to start building?[​](https://weaviate.io/blog/data-import-best-practices#ready-to-start-building "Direct link to Ready to start building?")
